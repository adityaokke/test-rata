...
import { FIND_OR_CREATE_ROOM_MUTATION, MY_ROOMS_QUERY } from '../graphql/chat.queries'
import { useAuth } from '../lib/auth-context'
import type { ChatRoom, FindOrCreateRoomResult, MyRoomsData, RoomParticipant } from '../types/chat.types'
...
export function RoomsPage() {
  ...
  const [findOrCreateRoom, { loading: creating }] = useMutation<FindOrCreateRoomResult>(
    ...
    <div className="min-h-screen bg-surface flex flex-col">
      ...
      <header className="border-b border-zinc-800 px-6 py-4 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-4">
          <span className="text-xs text-ink-muted font-mono">{user?.email}</span>
          {user?.role === 'AGENT' && (
            <button
          ...
          <button onClick={() => { logout(); navigate('/login') }} className="btn-ghost text-xs">
            Sign out
          </button>
        </div>
      </header>
      ...
      {showNewChat && (
        <Modal onClose={() => setShowNewChat(false)}>
          <h3 className="text-sm font-medium text-ink mb-4">Start a new conversation</h3>
          <form onSubmit={handleStartChat} className="flex flex-col gap-3">
            ...
            />
            <ModalActions
              onCancel={() => { setShowNewChat(false); setOtherUserId('') }}
          ...
          <p className="text-xs text-ink-faint mt-4 font-mono break-all">Your ID: {user?.id}</p>
        </Modal>
      )}

      <div className="flex-1 max-w-lg mx-auto w-full px-4 py-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-medium text-ink-muted uppercase tracking-wider">Messages</h2>
          <span className="text-xs text-ink-faint font-mono">{data?.myRooms.length ?? 0} rooms</span>
        </div>
        ...
        {data?.myRooms.map(room => {
          ...
          const initials = primary ? primary.userId.slice(0, 2).toUpperCase() : '??'
            ...
            >
              <div className="relative shrink-0">
                <div className="w-10 h-10 rounded-full bg-accent/10 border border-accent/20 flex items-center justify-center">
                  <span className="text-xs font-medium text-accent font-mono">{initials}</span>
                </div>
                {others.length > 1 && (
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  <p className="text-sm font-medium text-ink truncate font-mono">
                    {primary ? `${primary.userId.slice(0, 8)}…` : room.id.slice(0, 8)}
                  </p>
                  <span className="text-xs text-ink-faint shrink-0">{formatTime(room.updatedAt)}</span>
                </div>
                <div className="flex items-center gap-1.5 mt-0.5">
                  ...
                  {others.map(p => (
                    <span key={p.id} className={`text-[10px] px-1.5 py-0.5 rounded font-mono ${
                      p.role === 'CUSTOMER' ? 'bg-blue-950/50 text-blue-400' : 'bg-accent/10 text-accent'
                    }`}>
                      {p.role.toLowerCase()}
                    </span>
                  ))}
              ...
              <svg className="w-4 h-4 text-ink-faint shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
