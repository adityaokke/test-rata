...
import { MY_ROOMS_QUERY } from '../graphql/chat.queries'
...
export function RoomsPage() {
  ...
  function getOtherParticipant(room: ChatRoom) {
    return room.participantA === user?.id ? room.participantB : room.participantA
  }
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  <p className="text-sm font-medium text-ink truncate font-mono">{other.slice(0, 8)}…</p>
                  <span className="text-xs text-ink-faint shrink-0">{formatTime(room.updatedAt)}</span>
                </div>
                <p className="text-xs text-ink-muted mt-0.5">Click to open conversation</p>
              </div>
