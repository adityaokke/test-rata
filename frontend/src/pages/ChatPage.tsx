...
export function ChatPage() {
  ...
  const { optimistic, setOptimistic, sending, handleSend } = useSendMessage(roomId, user?.id)

  const [addAgent, { loading: addingAgent }] = useMutation(ADD_AGENT_MUTATION, {
    client: chatClient,
    onCompleted() { setShowAddAgent(false); setAgentId('') },
  ...
  useChatSocket({ roomId: roomId ?? '', userId: user?.id ?? '', onMessage: handleIncoming })
    ...
    <div className="min-h-screen bg-surface flex flex-col">
      ...
      <header className="border-b border-zinc-800 px-6 py-4 flex items-center gap-3 shrink-0">
        ...
        <div className="w-8 h-8 rounded-full bg-accent/10 border border-accent/20 flex items-center justify-center">
          <span className="text-xs font-mono text-accent">{roomId?.slice(0, 2).toUpperCase()}</span>
        </div>
        <div>
      ...
      {showAddAgent && (
        <Modal onClose={() => setShowAddAgent(false)}>
          ...
          <p className="text-xs text-ink-muted mb-4">Paste the user ID of the agent to add</p>
          <form onSubmit={e => { e.preventDefault(); if (agentId.trim()) addAgent({ variables: { input: { roomId, agentId: agentId.trim() } } }) }} className="flex flex-col gap-3">
            <input className="input-base" placeholder="Paste agent user ID…" value={agentId} onChange={e => setAgentId(e.target.value)} autoFocus />
            <ModalActions
              onCancel={() => { setShowAddAgent(false); setAgentId('') }}
      ...
      {/* Messages */}
      <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-2" onScroll={handleScroll}>
        {data?.messages.hasMore && (
          <div className="text-center py-2">
            <span className="text-xs text-ink-faint">Scroll up to load more</span>
          </div>
        )}
